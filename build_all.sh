#!/bin/bash
#
# Builds xmlwriter for distribution
...
#    MACOSX_DEPLOYMENT_TARGET=10.9 CC=clang CXX=clang++ python setup.py develop
    echo "---> Running python pip install -e . :"
    ...
    echo "---> Benchmarks:"
    echo "---> Cmd: pytest --benchmark-name=long --benchmark-sort=name --benchmark-save=benchmark_00 --benchmark-warmup=on tests/unit/"
    pytest --benchmark-name=long --benchmark-sort=name --benchmark-save=benchmark_00 --benchmark-warmup=on tests/unit/
...
#  python setup.py sdist
  echo "---> Running python -m build .  hopefully to create a sdist:"
  python -m build .
}
